...
public class BadControll : MonoBehaviour
{
    ...
    bool StrikePosible()
    {
        ...
        RaycastHit help;
        if (Physics.Raycast(myray, out help, sdist))
        {
            Debug.Log(help.point);
            if(help.collider.gameObject.GetComponent<HPcounter>() != null && help.collider.gameObject.GetComponent<HPcounter>().typeOb == 1)
                return true;
        }
        return false;
    }
    ...
    void Strike()
    {
        ...
        {
            Debug.Log(help.point);
            help.collider.gameObject.GetComponent<HPcounter>().GetStrike_Heall(spower);
        }
    }
